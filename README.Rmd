...
Conduct seasonal adjustment using X13-ARIMA-SEATS from R.  Almost all X13-ARIMA-SEATS functionality is supported, including the use of composite adjustments.

# Installation

If you have a public SSH key set up correctly with [GitLab](https://gitlabstats-prd):
...
A simple result viewer is also provided as a Shiny app.  For example, to view the `ap.res` object above, simply run:
...
Again, these are very simple interfaces, but they serve as a reasonable proof of concept which could easily be extended.
